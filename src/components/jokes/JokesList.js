...
import jokeUrl from '../../config/config';
        ...
        return (
            <Box className={classes.jokesList}>
                <Box className={classes.jokesListSidebar}>
                    ...
                    <img className={classes.sidebarImage} src='https://assets.dryicons.com/uploads/icon/svg/8927/0eb14c71-38f2-433a-bfc8-23d9c99b3647.svg' alt='laughing emoji' />
                </Box>
                <Box className={classes.jokesListJokes} >
                {jokes.map((joke) => {
                    return (
                        <Joke votes={joke.votes} text={joke.text} upvote={() => {handleVote(joke.id, 1);}} downvote={() => {handleVote(joke.id, -1);}} id={joke.id} />
                    )
                })}
